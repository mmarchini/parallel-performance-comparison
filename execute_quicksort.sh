...
for test_case in $(ls tests/qsort/); do
    ...
    go run $GO_SRC/quickSort.go tests/qsort/$test_case $LOOP results/go/sequential/qsort/$test_case
...
for test_case in $(ls tests/qsort/); do
    ...
    erl -noshell -s quickSort test_loop tests/qsort/$test_case $LOOP results/erlang/sequential/qsort/$test_case -s init stop

    # Parallel quick sort 
    echo "Running parallel quick sort $test_case"
    erl -noshell -s parallelQuickSort test_loop tests/qsort/$test_case $LOOP results/erlang/parallel/qsort/$test_case -s init stop
done
rm quickSort.beam 
